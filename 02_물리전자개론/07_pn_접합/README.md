...
위 식에서 $N_a, N_d$ 를 이전에는 도너 및 억셉터 불순물 농도라고 표기했었다. 여기서는 비슷하게 $N_a, N_d$는 각각 n과 p영역에서 순수 도너와 억셉터 농도를 의미한다. 예를들어 p영역이 보상물질(같은 영역에 도너와 억셉터 불순물을 함께 도핑한 물질)이라면 $N_a$는 실제의 억셉터와 도너 불순물농도간의 차를 나타내게된다.
...
위 식들을 통해 $-x_p \lt x \lt 0$ 에서의 전기장과 $0 \lt x \lt x_n$ 에서의 전기장 식을 각각 구하고 양 끝의 boundary 영역(각각 $-x_p, x_n$)에서 $E=0$ 조건을 넣어 미분방정식을 풀어서 나온 상수 값을 구할 수 있다. 이 식들을 그랲로 나타내면 위의 오른쪽 그림처럼 된다.
