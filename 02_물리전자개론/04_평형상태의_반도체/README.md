...
반대로 5족이 아니라 3족의 B(붕소)를 치환불순물로서 실리콘에 첨가했다고 생각해보자. 이 때 3족 원소는 가전자대로부터 전자를 받아드리므로 억셉터 불순물(acceptor impurity)원자라고 부른다. 억셉터 원자는 전도대에 전자를 생성하지 않고 가전자대에 정공을 생성한다. 이러한 형태의 반도체를 p형 반도체라고 부른다.
